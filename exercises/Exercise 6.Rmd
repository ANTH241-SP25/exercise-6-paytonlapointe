...
ggplot(BMIcat_barchart, aes(BMIcat, Freq)) + geom_col(fill="maroon")
```

c. Construct bar charts for one ordinal variable in the code chunk below:
```{r}
ggplot(NHANESdata, aes(x=HHEducation)) + geom_bar(fill = "lightblue") + labs(title="Education Level", x="Head of Household level of Education", y="counts")
...
ggplot(HHEducation_barchart, aes(HHEducation, Freq)) + geom_col(fill="maroon")
```
...
ggplot(NHANESdata, aes(x=BMIcat, fill= Gender)) + geom_bar(position = "stack") + facet_wrap(~Race) + labs(title="stacked bar chart", x="BMI categories by gender", y="counts")+  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=8))
ggplot(NHANESdata, aes(x=BMIcat, fill= Gender)) + geom_bar(position = "stack") + facet_grid(~Race) + labs(title="stacked bar chart", x="BMI categories by gender", y="counts") +  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=8))
#grouped bar chart:
#ggplot(data, aes(x=Independent variable, fill=dependent variable)) + geom_bar(position = "dodge") + labs(title="", x="Gender", y="Frequency")
ggplot(NHANESdata, aes(x=BMIcat, fill=Gender))+ geom_bar(position="dodge")+labs(title="grouped bar chart", x="BMI categories", y="Frequency")
```


e. Constructing bar charts for mean +/- standard error of continuous variables
```{r}
#1 generate means and standard deviations of BMI(dependent var.) by gender (independent var)
#new_data_frame <- data%>%
...
ggplot(BMI_gender, aes(x=reorder(Gender, mean), y=mean)) + 
  ...
  labs(x="Mean BMI by gender", y="Mean BMI")
```
...
ggplot(NHANESdata, aes(x=BMI)) + 
  geom_histogram(bins=20, color="white", fill="lightgreen") + 
  labs(title="BMI histogram", x="", y="")
```
...
ggplot(NHANESdata, aes(x=BMI)) + 
  ...
  labs(title="BMI Colored histogram with 20 bins", x="", y="")
```
...
ggplot(NHANESdata, aes(x=BMI, y=..density..)) + 
  geom_histogram(bins=20, color="white", fill="steelblue")+
  scale_y_continuous(labels=scales::percent) +
  labs(title="BMI", y="percent", x="BMI")
```

d: Create a histogram of a different continuous variable:
```{r}
ggplot(NHANESdata, aes(x=DR1TKCAL, y=..density..)) + 
  geom_histogram(bins=20, color="white", fill="steelblue")+
  scale_y_continuous(labels=scales::percent) +
  labs(title="Calories consumed daily", y="percent", x="number of calories consumed in one day")
```
...
ggplot(NHANESdata, aes(x="", y=DR1TPROT)) + 
  geom_boxplot() + 
  labs(y="Total proteins consumed daily in grams", x="", title="Box Plot")
```
...
ggplot(NHANESdata, aes(x=Race, y=DR1TPROT)) + geom_boxplot(fill="maroon") + labs(x="Race", y="Total proteins consumed daily in grams", title="daily protein intake in grams by race")
